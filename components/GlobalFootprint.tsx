...
const GET_GLOBAL_FOOTPRINT = gql`
  query GetGlobalFootprint {
    landingPage {
      video_title
      video_url
      video {
...
const DEFAULT_VIDEO = 'https://www.youtube.com/embed/__nmtRkzs9o?autoplay=1&mute=1&loop=1&playlist=__nmtRkzs9o&controls=0&showinfo=0&rel=0&modestbranding=1&iv_load_policy=3&disablekb=1&playsinline=1&enablejsapi=1';

const GlobalFootprint = () => {
  const { data } = useQuery<any>(GET_GLOBAL_FOOTPRINT, { errorPolicy: 'all' });

  const rawVideo = data?.landingPage?.video?.url
    ? getStrapiMediaUrl(data.landingPage.video.url)
    : data?.landingPage?.video_url
      ? getStrapiMediaUrl(data.landingPage.video_url)
      : DEFAULT_VIDEO;

  const title = data?.landingPage?.video_title || 'Hasan Group of Industries';
